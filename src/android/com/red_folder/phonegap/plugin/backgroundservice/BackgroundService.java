...
public abstract class BackgroundService extends Service {
    ...
    private final List<BackgroundServiceListener> mListeners = new ArrayList<BackgroundServiceListener>();
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        Log.d(TAG, "onStartCommand run: " + String.valueOf(intent.getAction()));
        initialiseService();
        return START_STICKY;
    }
     ...
     ************************************************************************************************
     */
    private final BackgroundServiceApi.Stub apiEndpoint = new BackgroundServiceApi.Stub() {
        ...
        public void removeListener(BackgroundServiceListener listener) throws RemoteException {
            synchronized (mListeners) {
                if (mListeners.size() > 0) {
                    ...
                    for (Iterator<BackgroundServiceListener> i = mListeners.iterator(); i.hasNext(); ) {
                        BackgroundServiceListener tmp = i.next();
                        if (listener.getUniqueID().equals(tmp.getUniqueID())) {
                            i.remove();
                            removed = true;
                        }
                    }
        ...
        return new TimerTask() {
            ...
            public void run() {
                try {
                    ...
                    Log.e(TAG, "Timer task error", ex);
                } finally {
                    Log.d(TAG, "Timer task completing work");
                }
            }
        };
    ...
    private void doWorkWrapper() {
        ...
        try {
            ...
            synchronized (mListeners) {
                ...
                for (BackgroundServiceListener listener : mListeners) {
                    try {
                        listener.handleUpdate();
                        Log.i(TAG, "Sent listener - " + listener.getUniqueID());
                    } catch (RemoteException e) {
                        Log.e(TAG, "Failed to send to listener - " + listener.toString() + " - " + e.getMessage(), e);
                    }
                }
            }
        } catch (Exception ex) {
            Log.e(TAG, "Exception occurred during doWork()", ex);
        }
